...
pygame.display.set_caption("WarVessels")
...
# Function to create a board with 2 grids for each player
def Board():
    board = []
    for i in range(2):
        player_grid = Grid()
...
# Function to get the number of missiles a player can shoot based on the dice roll
def get_num_missiles(dice_roll):
    if dice_roll == 1:
        return 0
    elif dice_roll >= 2 and dice_roll <= 5:
        return 1
    else:
        return 2

# Function to get the target coordinates for a player's missiles
def get_targets(num_missiles):
    targets = []
        ...
        while not valid_input:
            ...
            if x.isdigit() and y.isdigit():
                ...
                if x >= 0 and x <= 5 and y >= 0 and y <= 5:
                    ...
                    valid_input = True
            if not valid_input:
                print("Invalid input. Please enter a valid x and y coordinate between 0 and 5.")
    return targets
